...
extern "C"
{
	...
	/* Initializes a binary buddy memory allocator at the specified location using a non-default
	 * alignment */
	 ...
	 * Use to get a new handle to the allocator when the arena is moved or copied.
	 */
	struct buddy* buddy_get_embed_at_alignment(unsigned char* main, size_t memory_size,
											   size_t alignment);
	...
	void* buddy_reallocarray(struct buddy* buddy, void* ptr, size_t members_count,
							 size_t member_size, bool ignore_data);
	...
	enum buddy_safe_free_status buddy_safe_free(struct buddy* buddy, void* ptr,
	...
	/* Release a reserved memory range. Unsafe, this can mess up other allocations if called with
	 * wrong parameters! */
	 ...
	 *
	 * This will store a header at the start of the arena that contains the function pointer
	 * (tracker) and a void* (context). The tracker will be called with the context, the start of
	 * changed memory and its length.
	 *
	 * This function MUST be called before any allocations are performed!
	 *
	 * Change tracking is in effect only for allocation functions, resizing functions are excluded
	 * from it.
	 *
	 * This is an experimental feature designed to facilitate integration with
	 * https://github.com/spaskalev/libpvl
	 *
	 * The API is not (yet) part of the allocator contract and its semantic versioning!
	 */
	void buddy_enable_change_tracking(struct buddy* buddy, void* context,
...
typedef signed long ssize_t;
	...
	/* Returns the right adjacent node position or an invalid position if there is no right adjacent
	 * node */
	...
	static struct buddy_tree_interval buddy_tree_interval(struct buddy_tree* t,
	...
	static enum buddy_tree_release_status buddy_tree_release(struct buddy_tree* t,
															 struct buddy_tree_pos pos);
	...
	static void bitset_shift_right(unsigned char* bitset, size_t from_pos, size_t to_pos,
								   size_t by);
	...
	*/
	...
	static void buddy_toggle_range_reservation(struct buddy* buddy, void* ptr,
											   size_t requested_size, unsigned int state);
	...
	struct buddy* buddy_embed_alignment(unsigned char* main, size_t memory_size, size_t alignment)
	{
		...
		buddy =
			buddy_init_alignment(main + check_result.offset, main, check_result.offset, alignment);
		if(!buddy)
	...
	size_t buddy_arena_free_size(struct buddy* buddy)
	{
		...
		do
		{
			...
			if(pos_status == (tree_order - state.current_pos.depth + 1))
			{ /* Fully-allocated */
				state.going_up = 1;
			}
	...
	void* buddy_realloc(struct buddy* buddy, void* ptr, size_t requested_size, bool ignore_data)
	{
		 ...
		 * - Smaller size than previous *alloc decrease the allocated size with an optional
		 * rellocation
		 * - If the new allocation cannot be satisfied NULL is returned BUT the slot is preserved
		 * - Larger size than previous *alloc increase tha allocated size with an optional
		 * rellocation
		 */
		if(ptr == NULL)
		{
		...
		if(!ignore_data)
		{
			...
			memmove(
				destination, source,
				size_for_depth(buddy, current_depth > target_depth ? current_depth : target_depth));
		}
											   ...
											   size_t requested_size, unsigned int state)
	{
		...
		while(requested_size)
		{
			...
			requested_size =
				(requested_size < buddy->alignment) ? 0 : (requested_size - buddy->alignment);
			pos.index++;
		}
	...
	static struct internal_position buddy_tree_internal_position_order(size_t tree_order,
																	   struct buddy_tree_pos pos);
	static struct internal_position buddy_tree_internal_position_tree(struct buddy_tree* t,
																	  struct buddy_tree_pos pos);
	...
	static void write_to_internal_position(struct buddy_tree* t, struct internal_position pos,
										   size_t value);
	static size_t read_from_internal_position(unsigned char* bitset, struct internal_position pos);
	static inline unsigned char compare_with_internal_position(unsigned char* bitset,
															   struct internal_position pos,
															   size_t value);

#ifdef BUDDY_EXPERIMENTAL_CHANGE_TRACKING
	static inline void buddy_tree_track_change(struct buddy_tree* t, unsigned char* addr,
											   size_t length);
#endif /* BUDDY_EXPERIMENTAL_CHANGE_TRACKING */
	...
	static inline struct internal_position
		buddy_tree_internal_position_order(size_t tree_order, struct buddy_tree_pos pos)
	{
	...
	static inline struct internal_position
		buddy_tree_internal_position_tree(struct buddy_tree* t, struct buddy_tree_pos pos)
	{
		struct internal_position p;
		size_t total_offset, local_index;
		...
		while(desired_order > t->order)
		{
			...
			while(current_order)
			{
				...
				bitset_clear_range(buddy_tree_bits(t),
								   bitset_range(next_internal.bitset_location +
													(next_internal.local_offset * node_count),
												next_internal.bitset_location +
													(next_internal.local_offset * node_count * 2) -
													1));
	...
	static void buddy_tree_shrink(struct buddy_tree* t, uint8_t desired_order)
	{
		...
		while(desired_order < t->order)
		{
			...
			while(buddy_tree_valid(t, left_start))
			{
				...
				next_internal =
					buddy_tree_internal_position_order(next_order, buddy_tree_parent(left_start));
	...
	static void buddy_tree_populate_size_for_order(struct buddy_tree* t)
	{
		...
		{
			*((size_t*)(((unsigned char*)t) + sizeof(*t)) + t->size_for_order_offset + i) =
				size_for_order(t->order, (uint8_t)i);
		}
	}
										   ...
										   size_t value)
	{
		...
		struct bitset_range clear_range =
			bitset_range(pos.bitset_location, pos.bitset_location + pos.local_offset - 1);

		bitset_clear_range(bitset, clear_range);
		if(value)
		{
			bitset_set_range(bitset,
							 bitset_range(pos.bitset_location, pos.bitset_location + value - 1));
		}
	...
	static size_t read_from_internal_position(unsigned char* bitset, struct internal_position pos)
	{
		...
		return bitset_count_range(
			bitset, bitset_range(pos.bitset_location, pos.bitset_location + pos.local_offset - 1));
	}
	...
	static bool buddy_tree_interval_contains(struct buddy_tree_interval outer,
											 struct buddy_tree_interval inner)
	{
		return (inner.from.index >= outer.from.index) && (inner.from.index <= outer.to.index) &&
			   (inner.to.index >= outer.from.index) && (inner.to.index <= outer.to.index);
	}
									...
									struct internal_position pos_internal, size_t size_current)
	{
		...
		while(pos.index != 1)
		{
			pos_internal.bitset_location +=
				pos_internal.local_offset - (2 * pos_internal.local_offset * (pos.index & 1u));
	...
	static struct buddy_tree_pos buddy_tree_find_free(struct buddy_tree* t, uint8_t target_depth)
	{
		...
		while(current_depth != target_depth)
		{
			...
			if(compare_with_internal_position(tree_bits, left_internal, target_status + 1))
			{ /* left branch is busy, pick right */
				current_pos = right_pos;
			}
			else if(compare_with_internal_position(tree_bits, right_internal, target_status + 1))
	...
	static void buddy_tree_debug(struct buddy_tree* t, struct buddy_tree_pos pos, size_t start_size)
	{
		...
		do
		{
			...
			size_t pos_size = start_size >> ((buddy_tree_depth(state.current_pos) - 1u) %
		...
		do
		{
			...
			if(violated)
			{
				...
				BUDDY_PRINTF("current: %zu left %zu right %zu max %zu\n", current_status,
							 left_child_status, right_child_status, current_internal.local_offset);
			}
	...
	static unsigned char buddy_tree_fragmentation(struct buddy_tree* t)
	{
		...
		do
		{
			...
			{
				/* Empty node, process */
				virtual_size = two_to_the_power_of((tree_order - state.current_pos.depth) %
												   ((sizeof(size_t) * CHAR_BIT) - 1));
	...
	static size_t bitset_count_range(unsigned char* bitset, struct bitset_range range)
	{
		...
		if(range.from_bucket == range.to_bucket)
		{
			return popcount_byte(bitset[range.from_bucket] &
								 bitset_char_mask[range.from_index][range.to_index]);
		}
