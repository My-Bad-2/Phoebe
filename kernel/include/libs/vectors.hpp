...
struct _Vector_base
{
	...
	struct _Vector_impl : public _Tp_alloc_type, public _Vector_impl_data
	{
		...
		struct _Asan<allocator<_Up>>
		{
			...
			static _GLIBCXX20_CONSTEXPR void _S_grow(_Vector_impl& __impl, size_type __n)
			{
				_S_adjust(__impl, __impl._M_finish, __impl._M_finish + __n);
			}

			static _GLIBCXX20_CONSTEXPR void _S_shrink(_Vector_impl& __impl, size_type __n)
			{
			...
			struct _Reinit
			{
				explicit _GLIBCXX20_CONSTEXPR _Reinit(_Vector_impl& __impl) : _M_impl(__impl)
				{
			...
			{
				_GLIBCXX20_CONSTEXPR
				_Grow(_Vector_impl& __impl, size_type __n) : _M_impl(__impl), _M_n(__n)
				{
					_S_grow(_M_impl, __n);
				}
...
class vector : protected _Vector_base<_Tp, _Alloc>
{
  ...
  private:
	...
	static pointer _S_do_relocate(pointer, pointer, pointer __result, _Tp_alloc_type&,
								  false_type) noexcept
	{
		return __result;
	}

	static _GLIBCXX20_CONSTEXPR pointer _S_relocate(pointer __first, pointer __last,
													pointer __result,
 ...
 */
	 ...
	 */
	explicit _GLIBCXX20_CONSTEXPR vector(size_type __n,
	...
	_GLIBCXX20_CONSTEXPR
	vector(const vector& __x, const __type_identity_t<allocator_type>& __a) : _Base(__x.size(), __a)
	{
		this->_M_impl._M_finish = std::__uninitialized_copy_a(
			__x.begin(), __x.end(), this->_M_impl._M_start, _M_get_Tp_allocator());
	}
	...
	vector(vector&& __rv, const allocator_type& __m, false_type) : _Base(__m)
	{
		...
		else if(!__rv.empty())
		{
			...
			this->_M_impl._M_finish = std::__uninitialized_move_a(
				__rv.begin(), __rv.end(), this->_M_impl._M_start, _M_get_Tp_allocator());
			__rv.clear();
		}
	}
  ...
  public:
	...
	_GLIBCXX20_CONSTEXPR
	vector(vector&& __rv, const __type_identity_t<allocator_type>& __m) noexcept(
		noexcept(vector(std::declval<vector&&>(), std::declval<const allocator_type&>(),
 ...
 */
	...
	vector& operator=(vector&& __x) noexcept(_Alloc_traits::_S_nothrow_move())
	{
		constexpr bool __move_storage =
			_Alloc_traits::_S_propagate_on_move_assign() || _Alloc_traits::_S_always_equal();
 ...
 */
	 ...
	 *  see at().)
	 */
	_GLIBCXX_NODISCARD _GLIBCXX20_CONSTEXPR reference operator[](size_type __n) _GLIBCXX_NOEXCEPT
	{
  ...
  public:
	 ...
	 */
	_GLIBCXX_NODISCARD _GLIBCXX20_CONSTEXPR reference front() _GLIBCXX_NOEXCEPT
	{
	 ...
	 *  element of the %vector.
	 */
	_GLIBCXX_NODISCARD _GLIBCXX20_CONSTEXPR const_reference front() const _GLIBCXX_NOEXCEPT
	{
		__glibcxx_requires_nonempty();
		return *begin();
	}
	 ...
	 */
	_GLIBCXX_NODISCARD _GLIBCXX20_CONSTEXPR const_reference back() const _GLIBCXX_NOEXCEPT
	{
		__glibcxx_requires_nonempty();
		return *(end() - 1);
	}
	...
	_GLIBCXX_NODISCARD _GLIBCXX20_CONSTEXPR const _Tp* data() const _GLIBCXX_NOEXCEPT
	{
		return _M_data_ptr(this->_M_impl._M_start);
	}
	 ...
	 */
	template<typename... _Args>
	_GLIBCXX20_CONSTEXPR iterator emplace(const_iterator __position, _Args&&... __args)
	{
  ...
  protected:
	...
	_GLIBCXX20_CONSTEXPR pointer _M_allocate_and_copy(size_type __n, _ForwardIterator __first,
													  _ForwardIterator __last)
	{
		_Guard_alloc __guard(this->_M_allocate(__n), __n, *this);
		std::__uninitialized_copy_a(__first, __last, __guard._M_storage, _M_get_Tp_allocator());
	...
	void _M_initialize_dispatch(_Integer __int_n, _Integer __value, __true_type)
	{
		const size_type __n = static_cast<size_type>(__int_n);
		pointer __start = _M_allocate(_S_check_init_len(__n, _M_get_Tp_allocator()));
	...
	template<typename _InputIterator>
	_GLIBCXX20_CONSTEXPR void _M_range_initialize(_InputIterator __first, _InputIterator __last,
	...
	// Called by the second initialize_dispatch above
	template<typename _ForwardIterator>
	_GLIBCXX20_CONSTEXPR void _M_range_initialize(_ForwardIterator __first, _ForwardIterator __last,
												  std::forward_iterator_tag)
	{
		const size_type __n = std::distance(__first, __last);
		pointer __start = this->_M_allocate(_S_check_init_len(__n, _M_get_Tp_allocator()));
		_Guard_alloc __guard(__start, __n, *this);
		this->_M_impl._M_finish =
			std::__uninitialized_copy_a(__first, __last, __start, _M_get_Tp_allocator());
		this->_M_impl._M_start = __start;
	...
	{
		this->_M_impl._M_finish = std::__uninitialized_fill_n_a(this->_M_impl._M_start, __n,
	...
	// 438. Ambiguity in the "do the right thing" clause
	template<typename _Integer>
	_GLIBCXX20_CONSTEXPR void _M_assign_dispatch(_Integer __n, _Integer __val, __true_type)
	{
		_M_fill_assign(__n, __val);
	}
	...
	// Called by the range assign to implement [23.1.1]/9
	template<typename _InputIterator>
	_GLIBCXX20_CONSTEXPR void _M_assign_dispatch(_InputIterator __first, _InputIterator __last,
												 __false_type)
	{
		_M_assign_aux(__first, __last, std::__iterator_category(__first));
	}
	...
	// 438. Ambiguity in the "do the right thing" clause
	template<typename _Integer>
	_GLIBCXX20_CONSTEXPR void _M_insert_dispatch(iterator __pos, _Integer __n, _Integer __val,
												 __true_type)
	{
	...
	// Called by the range insert to implement [23.1.1]/9
	template<typename _InputIterator>
	_GLIBCXX20_CONSTEXPR void _M_insert_dispatch(iterator __pos, _InputIterator __first,
	...
	template<typename _InputIterator>
	_GLIBCXX20_CONSTEXPR void _M_range_insert(iterator __pos, _InputIterator __first,
											  _InputIterator __last, std::input_iterator_tag);

	// Called by the second insert_dispatch above
	template<typename _ForwardIterator>
	_GLIBCXX20_CONSTEXPR void _M_range_insert(iterator __pos, _ForwardIterator __first,
	...
	{
		template<typename... _Args>
		_GLIBCXX20_CONSTEXPR explicit _Temporary_value(vector* __vec, _Args&&... __args) :
			_M_this(__vec)
		{
	...
	template<typename... _Args>
	_GLIBCXX20_CONSTEXPR void _M_realloc_insert(iterator __position, _Args&&... __args);

	template<typename... _Args>
	_GLIBCXX20_CONSTEXPR void _M_realloc_append(_Args&&... __args);
	...
	template<typename... _Args>
	_GLIBCXX20_CONSTEXPR iterator _M_emplace_aux(const_iterator __position, _Args&&... __args);
	...
	static _GLIBCXX20_CONSTEXPR size_type _S_check_init_len(size_type __n,
															const allocator_type& __a)
	{
		if(__n > _S_max_size(_Tp_alloc_type(__a)))
			log_panik(__N("cannot create std::vector larger than max_size()"));
		return __n;
	}

	static _GLIBCXX20_CONSTEXPR size_type _S_max_size(const _Tp_alloc_type& __a) _GLIBCXX_NOEXCEPT
	{
 ...
 */
template<typename _Tp, typename _Alloc>
_GLIBCXX_NODISCARD _GLIBCXX20_CONSTEXPR inline bool operator==(const vector<_Tp, _Alloc>& __x,
...
template<typename _Tp, typename _Alloc>
[[nodiscard]]
constexpr __detail::__synth3way_t<_Tp> operator<=>(const vector<_Tp, _Alloc>& __x,
												   const vector<_Tp, _Alloc>& __y)
{
 ...
 */
template<typename _Tp, typename _Alloc>
_GLIBCXX_NODISCARD inline bool operator<(const vector<_Tp, _Alloc>& __x,
										 const vector<_Tp, _Alloc>& __y)
{
...
/// See std::vector::swap().
template<typename _Tp, typename _Alloc>
_GLIBCXX20_CONSTEXPR inline void swap(vector<_Tp, _Alloc>& __x, vector<_Tp, _Alloc>& __y)
