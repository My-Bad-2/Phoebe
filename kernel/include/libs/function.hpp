// Copied from libstdcxx's std::function

#ifndef LIBS_FUNCTION_HPP
#define LIBS_FUNCTION_HPP 1
...
{
template<typename T>
struct __is_location_invariant : std::is_trivially_copyable<T>::type
{
};
...
{
	void* _M_access() noexcept
	{
		return &_M_pod_data[0];
	}
	const void* _M_access() const noexcept
	{
		return &_M_pod_data[0];
	}
	...
	template<typename _Tp>
	const _Tp& _M_access() const noexcept
	{
	...
	class _Base_manager
	{
	  ...
	  public:
		...
		static void _M_init_functor(_Any_data& __functor, _Fn&& __f) noexcept(
			__and_<_Local_storage, is_nothrow_constructible<_Functor, _Fn>>::value)
		{
			_M_create(__functor, std::forward<_Fn>(__f), _Local_storage());
		}

		template<typename _Signature>
		static bool _M_not_empty_function(const function<_Signature>& __f) noexcept
		{
	...
	public _Function_base::_Base_manager<_Functor>
{
  ...
  public:
	...
	template<typename _Fn>
	static constexpr bool _S_nothrow_init() noexcept
	{
...
template<typename _Signature, typename _Functor, bool __valid = is_object<_Functor>::value>
struct _Target_handler : _Function_handler<_Signature, typename remove_cv<_Functor>::type>
{
};

template<typename _Signature, typename _Functor>
struct _Target_handler<_Signature, _Functor, false> : _Function_handler<void, void>
{
};
...
{
	template<typename _Func, bool _Self = is_same<__remove_cvref_t<_Func>, function>::value>
  ...
  public:
	 ...
	 *  (if it has one).
	 */
	function(function&& __x) noexcept : _Function_base(), _M_invoker(__x._M_invoker)
	{
		if(static_cast<bool>(__x))
		{
	...
	_Requires<_Callable<_Functor>, function&>
		operator=(_Functor&& __f) noexcept(_Handler<_Functor>::template _S_nothrow_init<_Functor>())
	{
...
using __function_guide_t = typename __conditional_t<requires(_Fn& __f) {
	(void)__f.operator();
}, __function_guide_static_helper<_Op>, __function_guide_helper<_Op>>::type;
